...
solution =
  let initialMemory = Data.Map.empty
      initialMask = map (const 'X') [1 .. 36]
      instructions = map parseInstruction input
   in ( Just . show $ sum $ map snd $ Data.Map.toList $ runV1 initialMemory initialMask instructions,
        Just . show $ sum $ map snd $ Data.Map.toList $ runV2 initialMemory initialMask instructions
      )

runV1 :: Memory -> Bitmask -> [Instruction] -> Memory
runV1 memory mask [] = memory
runV1 memory mask (instruction : instructions) = case instruction of
  WriteMemory address value ->
    let maskedValue = applyBitmask value mask
     in runV1 (writeMemory memory (address, maskedValue)) mask instructions
  SetBitmask nextMask -> runV1 memory nextMask instructions

applyBitmask :: Value -> Bitmask -> Value
applyBitmask value mask =
  let binaryValue = printf "%036b" value :: String
      maskedValue = zipWith applyCharMaskV1 mask binaryValue
   in toDecimalNumber maskedValue

applyCharMaskV1 :: Char -> Char -> Char
applyCharMaskV1 'X' value = value
applyCharMaskV1 '0' _ = '0'
applyCharMaskV1 '1' _ = '1'

runV2 :: Memory -> Bitmask -> [Instruction] -> Memory
runV2 memory mask [] = memory
runV2 memory mask (instruction : instructions) = case instruction of
  WriteMemory address value ->
    let memoryAddresses = findMemoryAddresses mask address
        nextMemory = foldl (\mem addr -> writeMemory mem (addr, value)) memory memoryAddresses
     in runV2 nextMemory mask instructions
  SetBitmask nextMask -> runV2 memory nextMask instructions

findMemoryAddresses :: Bitmask -> Address -> [Address]
findMemoryAddresses mask address =
  let binaryAddress = printf "%036b" address :: String
      floatingAddress = zipWith applyCharMaskV2 mask binaryAddress
   in sinkAddress floatingAddress

sinkAddress :: FloatingAddress -> [Address]
sinkAddress address =
  let maybeFloatIndex = elemIndex 'X' address
   in case maybeFloatIndex of
        Nothing -> [toDecimalNumber address]
        Just index ->
          let zero = sinkAddress (replaceAt address index '0')
              one = sinkAddress (replaceAt address index '1')
           in zero ++ one

replaceAt :: [a] -> Int -> a -> [a]
replaceAt xs index value = Data.Foldable.toList $ Data.Sequence.update index value $ Data.Sequence.fromList xs

applyCharMaskV2 :: Char -> Char -> Char
applyCharMaskV2 'X' _ = 'X'
applyCharMaskV2 '0' value = value
applyCharMaskV2 '1' _ = '1'

toDecimalNumber :: String -> Int
toDecimalNumber = foldl' (\acc x -> acc * 2 + digitToInt x) 0

writeMemory :: Memory -> (Address, Value) -> Memory
writeMemory memory (address, value) = Data.Map.insert address value memory

parseInstruction :: String -> Instruction
parseInstruction s =
  if s =~ "^mask" :: Bool
    then
      let (_, _, _, subMatches) = s =~ "^mask = ([X10]+)" :: (String, String, String, [String])
       in SetBitmask $ head subMatches
    else
      let (_, _, _, subMatches) = s =~ "^mem\\[(\\d+)\\] = (\\d+)" :: (String, String, String, [String])
          address = read $ subMatches !! 0
          value = read $ subMatches !! 1
       in WriteMemory address value

data SetBitmask bitmask

data WriteMemory address value

data Instruction = WriteMemory Address Value | SetBitmask Bitmask deriving (Show)

type Bitmask = String

type Memory = Data.Map.Map Int Int

type FloatingAddress = String

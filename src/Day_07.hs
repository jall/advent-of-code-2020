...
solution =
  let rules = map parseRule $ lines input
   in ( Just
          . show
          . length
          $ findParentColours rules ["shiny gold"] [],
        -- Subtract 1 because we only want the children of the shiny gold bag
        Just . show $ (countBags rules "shiny gold") - 1
      )

countBags :: [Rule] -> String -> Int
countBags rules colour =
  let dictionary = fromList rules
      children = dictionary ! colour
   in if null children
        then 1
        else (+ 1) . sum $ map (\(amount, childColour) -> amount * (countBags rules childColour)) children

findParentColours :: [Rule] -> [String] -> [String] -> [String]
findParentColours rules colours existingParents =
  let newDirectParents = concat $ map (findDirectParentsOfColour rules) colours
      newParents = nub (existingParents ++ newDirectParents)
   in if null (newParents \\ existingParents)
        then newParents
        else
          findParentColours
            rules
            (nub (colours ++ newDirectParents))
            newParents

findDirectParentsOfColour :: [Rule] -> String -> [String]
findDirectParentsOfColour rules colour =
  map fst $ filter (\(_, children) -> any (\(_, childColour) -> childColour == colour) children) rules

parseRule :: String -> Rule
parseRule s =
  let (_, _, childString, matches) = s =~ "^(\\w+ \\w+) bags contain " :: (String, String, String, [String])
      parent = matches !! 0
      children = catMaybes . (map parseChild) . (splitOn ", ") $ childString
   in (parent, children)

parseChild :: String -> Maybe (Int, String)
parseChild s =
  let (_, _, _, matches) = s =~ "(\\d+) (\\w+ \\w+) bags?" :: (String, String, String, [String])
   in if null matches
        then Nothing
        else
          let amount = read (matches !! 0) :: Int
              colour = matches !! 1
           in Just (amount, colour)

type Rule = (String, [Child])

type Child = (Int, String)
...
input =
  [r|mirrored gold bags contain 3 light teal bags.

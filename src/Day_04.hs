...
solution =
  let passports = (map toPassport) $ splitOn "\n\n" input
   in ( Just $ show . length . (filter hasMandatoryFields) $ passports,
        Just $ show . length . (filter isValidPassport) $ passports
      )

toPassport :: String -> Passport
toPassport s =
  let withoutNewLines = replace "\n" " " s; chunks = splitOn " " withoutNewLines
   in map
        ( \s ->
            let (field, _, value) = s =~ ":" :: (String, String, String)
             in (field, value)
        )
        chunks
...
isValidPassport passport =
  let validFields = filter isValidField passport
      validCount = (length validFields)
   in validCount == 7
...
isValidField (field, value)
  | field == "byr" =
    if value =~ "^[0-9]{4}$" :: Bool
      then let number = read value in 1920 <= number && number <= 2002
      else False
  | field == "iyr" =
    if value =~ "^[0-9]{4}$" :: Bool
      then let number = read value in 2010 <= number && number <= 2020
      else False
  | field == "eyr" =
    if value =~ "^[0-9]{4}$" :: Bool
      then let number = read value in 2020 <= number && number <= 2030
      else False
  | field == "hgt" =
    let (_, match, _, subMatches) = value =~ "^([0-9]+)(in|cm)$" :: (String, String, String, [String])
     in if match /= ""
          then
            let number = read (subMatches !! 0)
                isCm = (subMatches !! 1) == "cm"
             in if isCm
                  then 150 <= number && number <= 193
                  else 59 <= number && number <= 76
          else False
  | field == "hcl" = value =~ "^#[0-9a-f]{6}$" :: Bool
  | field == "ecl" = value == "amb" || value == "blu" || value == "brn" || value == "gry" || value == "grn" || value == "hzl" || value == "oth"
  | field == "pid" = value =~ "^[0-9]{9}$" :: Bool
  | otherwise = False

hasMandatoryFields :: Passport -> Bool
hasMandatoryFields passport =
  let fields = map fst passport; mandatoryFields = ["byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid"]
   in (length (intersect fields mandatoryFields)) == length mandatoryFields

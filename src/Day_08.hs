...
solution =
  ( Just $ fromRight "error" $ mapRight show $ run input [] 0 0,
    Just $
      fromLeft "error" $
        mapLeft show $
...
mutateOnce instructions indexToMutate =
  let (prefixInstructions, (opCode, sign, value) : suffixInstructions) = splitAt indexToMutate instructions
...
run instructions visitedPositions accumulator currentPosition
  | currentPosition `elem` visitedPositions = Right accumulator
  | otherwise =
    case instructions ^? element currentPosition of
      Nothing -> Left accumulator
      Just (opCode, sign, value) ->
        let nextPosition =
              if opCode == "jmp"
                then case sign of
...
type Instruction = (String, String, Int)

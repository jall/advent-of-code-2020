...
solution =
  let groupAnswers = map (splitOn "\n") $ splitOn "\n\n" input
   in ( Just $ show . sum . (map length) $ map questionsAnyoneAnswered groupAnswers,
        Just $ show . sum . (map length) $ map questionsEveryoneAnswered groupAnswers
      )

questionsAnyoneAnswered :: [String] -> String
questionsAnyoneAnswered = nub . concat

questionsEveryoneAnswered :: [String] -> String
questionsEveryoneAnswered = foldr1 intersect
